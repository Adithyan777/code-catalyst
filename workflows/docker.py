...
from .normal import CommandResponse
from tools.ask_user import ask_human
import re

monitor_agents: bool = True

class DockerSetupWorkflow(Workflow):
    apikey: str = Field(...)
    ...
    def __init__(self, apikey: str, console: Console, **data):
        ...
        self.extractor = Agent(
            ...
            system_prompt=PromptManager.get_prompt("extract_info_agent", setup="docker"),
            tools=[ask_human],
        ...
        self.template_agent = Agent(
            name="template_agent",
            model=OpenAIChat(model='gpt-4o-mini', api_key=self.apikey),
            system_prompt=PromptManager.get_prompt("template_agent", setup="docker"),
        ...
        self.tester_agent = Agent(
            name="tester_agent",
            model=OpenAIChat(model='gpt-4o-mini', api_key=self.apikey),
            system_prompt=PromptManager.get_prompt("tester_agent", setup="docker"),
            structured_outputs=True,
            output_model=CommandResponse,
            monitoring=monitor_agents
        )

        # add docker specific agents here
    ...
    def run(self,project_name: str,project_description: str) -> Iterator[RunResponse]:
        ...
        # Define the regex pattern to match the project information section from 1-7
        pattern = r'1\. \*\*Project Name\*\*:.*?(?=\n\s*TERMINATE|$)'
        ...
        # ----------------------------------------
        # STEP 3 : TESTER AGENT (TODO: rethink the order of agents)
        # ----------------------------------------
